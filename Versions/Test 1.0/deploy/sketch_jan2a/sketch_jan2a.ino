...
// state machine looks like 1 -> 2 (5 -> 4 -> 6 (5 and 6 could repeat)) -> 3 (5) -> end
...
// #define PATH_CONSTRUCTION_SUBSTATE 4 path construction is not available for the version 1.0, only course correction
...
short DESCEND_SUBSTATE = COURSE_CORRECTION_SUBSTATE; // descend substate is initialized only for course correction as path following is not available in test 1.0
bool fallDetected = false;

// threads
int baroThr; //due to the sensor fusion algorithm repeptitive and uninterruptive nature, the data fetching is happening on the other thread.
...
float nextPointHeading = 0.0; // heading to the next point (determined between current and next point) not available in test 1.0

// Modelling
int modelLatitude = 0;
int modelLongitude = 0;

// Magnetometer debug variables
...
// Accelerometer (used to measure free fall and landed state)
...
float temperature_baro = 0.0; // Degrees Centigrade 
...
// initial location will be determined at the beginning of the path construction not available in test 1.0
float inLat = 0.000000;
float inLong = 0.000000;

// suggested point along the path, will be suggested based on the line equation not available in test 1.0
...
// not available in test 1.0
float m = 0.0;
float b = -190.0;
// inverted line coefficients
float m1 = 0.0;
float b1 = -190.0;

bool invert = false; // triggered true when it is impossible to describe a line using a parametric func, as it is vertical, thus the coordinate system is flipped. not available in test 1.0
...
PID pathCorrection(&pidDeltaDistanceInput, &steeringOutput, &allowedGoalFollowing, fP, fI, fD, DIRECT); /// init path following PID not available in test 1.0
...
float distance2Points(float lat1, float lon1, float lat2, float lon2) // todo. DOes not estimate the distance correctly
{
  // lon1 = lon1 / 180 * PI;
  // lat1  = lat1 / 180 * PI;
  // lon2 = lon2 / 180 * PI;
...
void accelThread() // the function is being called by main loop
{
...
bool detectFall()
{
  ...
  // if(vspeed<0 && pAcc < 0.05 && gnssVspeed < 0) // todo adjust thresholds to increase detection rate. Better to have false positive than true negative. 
  // {
  //   state = DESCEND_STATE; //turns state into descend, where parachute is deployed and steering takes place 
  //   return true;
  // }
  if(pAcc < 0.05)
  {
...
void stateAction() //todo finish the state machine, develop descend state machine.
{
  switch(state)
  {
    ...
    case ASCEND_STATE: //fall detection is used in the accelerometer thread. To have a higher rate of detection. 
    {
      //todo do measurements and calculations of groundspeed and thus windspeed
    } break;
    case DESCEND_STATE:
    {
      gnssThread();
